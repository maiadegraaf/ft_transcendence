...
export class ChannelService {
    ...
    async newDmChannel(user1: User, user2: User): Promise<any> {
        try {
            ...
            if (!channel) {
                throw new HttpException('Could not create new dm channel', HttpStatus.FORBIDDEN)
            }
            channel.users = []
    ...
    async checkValidNewDm(user1: User, user2: User): Promise<any> {
        if (user1.id === user2.id) {
            throw new HttpException('Cannot create dm with yourself', HttpStatus.FORBIDDEN)
        }
        ...
        if (channels) {
            for (const channel of channels) {
                ...
                if (userIds.includes(user1.id) && userIds.includes(user2.id)) {
                    throw new HttpException('Dm already exists', HttpStatus.FORBIDDEN)
                }
            }
        }
    }
    ...
    async getUserChannels(userId: number): Promise<Channel[]> {
        ...
        const blockedUsers = await this.userService.getBlockedUsersForUser(userId)
        if (!blockedUsers) {
            return channels
        }
        for (const channel of channels) {
    ...
    async newGroupChannel(
        ...
        const channel = await this.createChannel()
        if (!channel) {
            throw new HttpException('could not create channel', HttpStatus.FORBIDDEN)
            return
        }
        ...
        if (!groupProfile) {
            throw new HttpException('Could not create new group profile', HttpStatus.FORBIDDEN)
        }
    ...
    async removeUserFromChannel(channelId: number, user: User): Promise<any> {
        ...
        if (idx === -1) {
            throw new HttpException('User not found in channel to remove', HttpStatus.FORBIDDEN)
        }
    ...
    async isSenderValidatedReturnChannel(payload: MessageDto): Promise<any> {
        ...
        if (channel.profile.muted.find((u) => u.id === payload.sender.id)) {
            throw new HttpException(
                'isSenderValidatedReturnChannel: Sender with id' + payload.sender.id + ' is muted',
                HttpStatus.FORBIDDEN
            )
        }
        return channel
