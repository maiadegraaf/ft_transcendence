...
export class GroupProfileService {
    ...
    async newGroupProfile(
        ...
        if (!groupProfile) {
            throw new HttpException('could not create group profile', HttpStatus.FORBIDDEN)
            return
        }
    ...
    async addAdmin(param: GroupUserProfileUpdateDto, userId: number): Promise<any> {
    ...
    async addBlocked(param: GroupUserProfileUpdateDto, userId: number): Promise<any> {
    ...
    async addMute(param: GroupUserProfileUpdateDto, userId: number): Promise<any> {
    ...
    async deleteAdmin(param: GroupUserProfileUpdateDto, userId: number): Promise<any> {
        const group = await this.ownerCheck(param, userId)
        const user = await this.userGroupProfileCheck(group, param)
    ...
    async deleteBlocked(param: GroupUserProfileUpdateDto, userId: number): Promise<any> {
        const group = await this.adminCheck(param, 'blocked', userId)
        const user = await this.userGroupProfileCheck(group, param)
        const idx = group.blocked.findIndex((blocked) => blocked.id === user.id)
        if (idx === -1) {
            throw new HttpException('admin not in group to delete', HttpStatus.FORBIDDEN)
        }
    ...
    async deleteMute(param: GroupUserProfileUpdateDto, userId: number): Promise<any> {
        const group = await this.adminCheck(param, 'muted', userId)
        const user = await this.userGroupProfileCheck(group, param)
    ...
    async ownerCheck(param: GroupUserProfileUpdateDto, userId: number): Promise<any> {
        const group = await this.groupProfileRepository
            .createQueryBuilder('group')
            ...
            .getOne()
        if (!group) {
            throw new HttpException('could not find group due to wrong id', HttpStatus.FORBIDDEN)
        }
    ...
    async userGroupProfileCheck(
        ...
        const user = group.channel.users.find((usr) => usr.login === param.userName)
        if (!user) {
            throw new HttpException('could not find user to add to admin', HttpStatus.FORBIDDEN)
        }
        return user
    }

    async adminCheck(param: GroupUserProfileUpdateDto, type: string, userId: number): Promise<any> {
        const group = await this.groupProfileRepository
            .createQueryBuilder('group')
            .where('group.id = :id', { id: param.groupId })
            .leftJoinAndSelect('group.admin', 'admin')
        ...
        if (!group.admin.find((admin) => admin.id === userId)) {
            throw new HttpException('user is not an admin in group', HttpStatus.FORBIDDEN)
        }
        return group
    }
    ...
    async isBlocked(userId: number, groupId: number): Promise<boolean> {
        const group = await this.groupProfileRepository
            ...
            .getOne()
        if (!group) {
            throw new HttpException('could not find group in isBlocked', HttpStatus.FORBIDDEN)
        }
    ...
    async newPassword(group: GroupProfile, newPassword: string): Promise<GroupProfile> {
        const hash = await bcrypt.hash(newPassword, 10)
        group.password = hash
        return await this.groupProfileRepository.save(group)
    }

    async validatePassword(groupProfile: GroupProfile, password: string): Promise<boolean> {
