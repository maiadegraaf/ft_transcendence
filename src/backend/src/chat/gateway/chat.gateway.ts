...
export class ChatGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    handleDisconnect(@ConnectedSocket() client: Socket) {
        if (!client.request.session.user) {
            this.clientMap.forEach((value, key) => {
                if (value.id === client.id) {
                    ...
                    this.logger.log(`Client disconnected to chat: ${client.id} with userId: ${key}`)
                }
            })
            return
        }
        ...
        if (this.clientMap.has(parseInt(userId.toString()))) {
            ...
            this.logger.log(`Client disconnected to chat: ${client.id} with userId: ${userId}`)
        }
    }
    ...
    async emitNewDmChannel(user1: User, user2: User, channel: Channel): Promise<any> {
        const user1Socket = this.getClientSocketById(user1.id)
        if (user1Socket) {
            this.logger.log('emit addChannelToClient form user1: ' + user1Socket.id)
            user1Socket.emit('addChannelToClient', channel)
        }
        const user2Socket = this.getClientSocketById(user2.id)
        if (user2Socket) {
            this.logger.log('emit addChannelToClient form user2: ' + user2Socket.id)
    ...
    async emitGroupChannelToUser(channel: Channel, user: User): Promise<any> {
        ...
        if (userSocket) {
            this.logger.log('emit addChannelToClient to user: ' + userSocket.id)
    ...
    async emitDeleteChannelFromUser(channel: Channel, user: User): Promise<any> {
        try {
            ...
            this.logger.log(
                'emit removeChannelFromClient user: ' + userSocket.id + ' with userId: ' + user.id
            )
        } catch (error) {
            this.logger.error(error)
        }
    ...
    async emitRemoveAdminFromChannel(info: {
        ...
        this.server.to('room' + info.channelId).emit('removeAdminFromChannel', info)
    ...
    async emitAddMutedToChannelToUser(info: {
        ...
        this.logger.log('emitAddMutedToChannelToUser for user: ' + info.user.id)
    }
    ...
    async emitRemoveMutedFromChannelToUser(info: {
        ...
        this.server.to('room' + info.channelId).emit('removeMutedFromChannel', info)
        this.logger.log('emitRemoveMutedFromChannelToUser for user: ' + info.user.id)
    }
}
