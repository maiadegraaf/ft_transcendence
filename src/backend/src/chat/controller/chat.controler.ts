...
export class ChatController {
    ...
    async getUserChannel(@Param('id') userId: number): Promise<any> {
        ...
        if (!channels) {
            this.logger.error('getUserChannels: No channels found from user: ' + userId)
            return
        }
    ...
    // Post /api/chat/dm
    @Post('dm')
    async postNewDMChannel(@Body(new ValidationPipe()) param: CreateDmChannelDto): Promise<any> {
        try {
