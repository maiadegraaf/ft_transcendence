...
export class MatchService {
    constructor(@InjectRepository(Matches) private MatchRepository: Repository<Matches>) {}
    ...
    async findMatchesByPlayerAndOpponent(player: User, opponent: User): Promise<Matches[]> {
        return this.MatchRepository.find({
            select: ['player1Score', 'player2Score'],
            relations: {
                player1: true,
